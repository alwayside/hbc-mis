      ...
      <v-card-text>
        <v-row>
          ...
          <v-col>
            ...
            <span class="title">{{$t('medicinePurchase')}}</span>
          </v-col>
          <v-col sm="1"></v-col>
        </v-row>
        <v-row>
...
export default {
  ...
  methods: {
    getDate () {
      ...
      let url = '/Purchase?purchaseType=' + this.selectType + '&purchaseStore=' + this.selectStore + '&purchaseNumber=' + this.number
      this.$axios
        .put(url)
        .then(res => {
          if (res.status === 200) {
